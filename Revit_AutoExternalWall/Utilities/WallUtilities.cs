                        ...
                        try
                        {
                            ...
                            if (insertionPoint != null)
                            {
                                ...
                                if (newOpening != null)
                                {
                                    ...
                                    // Пост-проверка и корректировка позиции окна
                                    doc.Regenerate(); // Регенерируем, чтобы получить актуальную позицию нового окна
                                    
                                    try
                                    {
                                        // Получаем текущую позицию нового окна
                                        LocationPoint newLocationPoint = newOpening.Location as LocationPoint;
                                        LocationCurve newLocationCurve = newOpening.Location as LocationCurve;
                                        
                                        XYZ newOpeningPosition = null;
                                        if (newLocationPoint != null)
                                        {
                                            newOpeningPosition = newLocationPoint.Point;
                                        }
                                        else if (newLocationCurve != null && newLocationCurve.Curve != null)
                                        {
                                            newOpeningPosition = newLocationCurve.Curve.Evaluate(0.5, true);
                                        }

                                        if (newOpeningPosition != null && openingAbsolutePosition != null)
                                        {
                                            // Вычисляем смещение между исходным и новым окном
                                            XYZ offset = openingAbsolutePosition - newOpeningPosition;
                                            
                                            // Вычисляем расстояние смещения в горизонтальной плоскости (XY)
                                            double horizontalOffset = Math.Sqrt(offset.X * offset.X + offset.Y * offset.Y);
                                            
                                            // Если смещение больше допустимого (например, 0.05 фута = ~15 мм), корректируем
                                            const double tolerance = 0.05; // ~15 мм
                                            if (horizontalOffset > tolerance)
                                            {
                                                // Проецируем исходную позицию окна на кривую внешней стены
                                                LocationCurve externalWallLocation = bestExternalWall.Location as LocationCurve;
                                                if (externalWallLocation != null && externalWallLocation.Curve != null)
                                                {
                                                    Curve externalCurve = externalWallLocation.Curve;
                                                    double correctedParam = externalCurve.Project(openingAbsolutePosition).Parameter;
                                                    XYZ correctedPoint = externalCurve.Evaluate(correctedParam, true);
                                                    
                                                    // Перемещаем окно на правильную позицию
                                                    if (newLocationPoint != null)
                                                    {
                                                        newLocationPoint.Point = correctedPoint;
                                                        Log(doc, $"Скорректирована позиция окна/двери {newOpening.Id}: смещение было {horizontalOffset:F3} футов");
                                                    }
                                                    else if (newLocationCurve != null && newLocationCurve.Curve != null)
                                                    {
                                                        // Для LocationCurve нужно переместить кривую
                                                        Curve currentCurve = newLocationCurve.Curve;
                                                        XYZ currentMid = currentCurve.Evaluate(0.5, true);
                                                        XYZ moveVector = correctedPoint - currentMid;
                                                        
                                                        // Создаем новую кривую со смещением
                                                        Curve newCurve = currentCurve.CreateTransformed(Transform.CreateTranslation(moveVector));
                                                        newLocationCurve.Curve = newCurve;
                                                        Log(doc, $"Скорректирована позиция окна/двери {newOpening.Id}: смещение было {horizontalOffset:F3} футов");
                                                    }
                                                }
                                            }
                                            ...
                                            {
                                                Log(doc, $"Позиция окна/двери {newOpening.Id} корректна (смещение: {horizontalOffset:F3} футов)");
                                            }
                                        }
                                    }
                                    catch (Exception adjustEx)
                                    {
                                        Log(doc, $"Ошибка при корректировке позиции окна/двери {newOpening.Id}: {adjustEx.Message}");
                                    }
